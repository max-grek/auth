...
import           Data.List                  (elemIndex, group, intercalate,
                                             isPrefixOf, sort)
...
_splitBy x xs = (`splitAt` xs) <$> elemIndex x xs

_splitByAndRemove :: Eq a => a -> [a] -> Maybe ([a],[a])
_splitByAndRemove x xs = fmap tail <$> _splitBy x xs -- fmap (fmap tail) $ splitBy x xs

_splitBy2 :: Eq a => a -> [a] -> Maybe ([a],[a])
_splitBy2 x xs = case splitOn [x] xs of
              (x:y:_) -> Just (x, y)
              (x:_)   -> Nothing

-- _splitByAndRemove2 :: Eq a => a -> [a] -> Maybe ([a],[a])
-- _splitByAndRemove2 x xs = splitted  >>
--   s <- splitOn [x] xs
--   case s of
--     []     -> Nothing
--     (y:_)  -> Nothing
...
--   where
--     splitted = splitOn [x] xs -- fmap (fmap tail) $ splitBy x xs
...
parse flags args = do
  ...
  pure $ merge name_value_pair $ map (fromMaybe ("","") . _splitBy2 '=' . drop 2) args
  where
